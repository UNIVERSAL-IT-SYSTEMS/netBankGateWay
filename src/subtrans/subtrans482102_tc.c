...
int ics_proc_482102_tc(char *send_buff,char *recv_buff)
{
  /*说明：send_buff-上传报文；
          recv_buff-下传报文;
 ...
 */
  ...
  ICS_DEF_482102_I_TC  *pICS_482102_I;
  ICS_DEF_482102_N_TC  *pICS_482102_N;
  ICS_DEF_482102_E_TC  *pICS_482102_E;

  char      ics_send_buff[LEN_ICS_PROC_BUF];
  char      ics_recv_buff[LEN_ICS_PROC_BUF];
  char      ics_482102i_buff[38];
  char      ics_482102n_buff[118];
  char      ics_482102e_buff[75];
  ...
  pICS_482102_I=(ICS_DEF_482102_I_TC *)ics_482102i_buff;
  pICS_482102_N=(ICS_DEF_482102_N_TC *)ics_482102n_buff;
  pICS_482102_E=(ICS_DEF_482102_E_TC *)ics_482102e_buff;
...
flog( STEP_LEVEL,"--482102 接收[%s]-------------------------------",send_buff);
 ...
 /* 注意：填充数据最好按照结构定义先后顺序，以免出现数据覆盖问题 */
  ...
  /* STEP1-3: 填上传串中的固定元素值*/
  /* 格式: strcpy(pICS_482102_I->RsFld1,"P001"); 第三方交易码(查询)*/

  /* STEP1-4: 从上传报文中获得其余值-用getValueOf函数*/
	memset(tmpvalue, 0, sizeof(tmpvalue));
  getValueOfStr(send_buff,"TikMod",tmpvalue); /*购票方式   */
  strcpy(pICS_482102_I->TikMod,tmpvalue);

	memset(tmpvalue, 0, sizeof(tmpvalue));
  getValueOfStr(send_buff,"SigDup",tmpvalue); /*单复式区分 */
  strcpy(pICS_482102_I->SigDup,tmpvalue);

	memset(tmpvalue, 0, sizeof(tmpvalue));
  getValueOfStr(send_buff,"NotNum",tmpvalue); /*机选注数   */
  strcpy(pICS_482102_I->NotNum,tmpvalue);

	memset(tmpvalue, 0, sizeof(tmpvalue));
	memset(pICS_482102_I->LotNum, ' ', sizeof(pICS_482102_I->LotNum));
  getValueOfStr(send_buff,"LotNum",tmpvalue); /*投注号码   */
  strcpy(pICS_482102_I->LotNum,tmpvalue);

	memset(tmpvalue, 0, sizeof(tmpvalue));
  getValueOfStr(send_buff,"LotTyp",tmpvalue); /*彩票类型   */
  strcpy(pICS_482102_I->LotTyp,tmpvalue);

	memset(tmpvalue, 0, sizeof(tmpvalue));
  getValueOfStr(send_buff,"MulTip",tmpvalue); /*倍数       */
  strcpy(pICS_482102_I->MulTip,tmpvalue);

	memset(tmpvalue, 0, sizeof(tmpvalue));
  getValueOfStr(send_buff,"ExtNum",tmpvalue); /*扩展号码   */
  strcpy(pICS_482102_I->ExtNum,tmpvalue);
  ...
  len=sizeof(ICS_DEF_482102_I_TC);
  for(i=0;i<len;i++)
  {
  ...
  offset=offset+sizeof(ICS_DEF_482102_I_TC);
  ...
  if(memcmp(pICS_TOA->RspCod,"000000",sizeof(pICS_TOA->RspCod))==0)/*成功*/
  {
    ...
    memcpy(pICS_482102_N,ics_recv_buff+sizeof(ICS_DEF_TOA),sizeof(ICS_DEF_482102_N_TC));
    ...
    /* 调用setValueOf函数填充 */
      ...
      memset(tmp_val_str,'\0',sizeof(tmp_val_str));
      memcpy(tmp_val_str,pICS_482102_N->TmpDat,sizeof(pICS_482102_N->TmpDat));
      setValueOfStr(recv_buff,"TmpDat",tmp_val_str);/*包体长度*/

      memset(tmp_val_str,'\0',sizeof(tmp_val_str));
      memcpy(tmp_val_str,pICS_482102_N->ApCode,sizeof(pICS_482102_N->ApCode));
      ...
      memset(tmp_val_str,'\0',sizeof(tmp_val_str));
      memcpy(tmp_val_str,pICS_482102_N->TRspCd,sizeof(pICS_482102_N->TRspCd));
      setValueOfStr(recv_buff,"TRspCd",tmp_val_str);/*状态*/

      memset(tmp_val_str,'\0',sizeof(tmp_val_str));
      memcpy(tmp_val_str,pICS_482102_N->NotNum,sizeof(pICS_482102_N->NotNum));
      setValueOfStr(recv_buff,"NotNum",tmp_val_str);/*注数       */

      memset(tmp_val_str,'\0',sizeof(tmp_val_str));
      memcpy(tmp_val_str,pICS_482102_N->LotNum1,sizeof(pICS_482102_N->LotNum1));
      setValueOfStr(recv_buff,"LotNum1",tmp_val_str);/*注1*/

      memset(tmp_val_str,'\0',sizeof(tmp_val_str));
      memcpy(tmp_val_str,pICS_482102_N->LotNum2,sizeof(pICS_482102_N->LotNum2));
      setValueOfStr(recv_buff,"LotNum2",tmp_val_str);/*注2*/

      memset(tmp_val_str,'\0',sizeof(tmp_val_str));
      memcpy(tmp_val_str,pICS_482102_N->LotNum3,sizeof(pICS_482102_N->LotNum3));
      setValueOfStr(recv_buff,"LotNum3",tmp_val_str);/*注3*/

      memset(tmp_val_str,'\0',sizeof(tmp_val_str));
      memcpy(tmp_val_str,pICS_482102_N->LotNum4,sizeof(pICS_482102_N->LotNum4));
      setValueOfStr(recv_buff,"LotNum4",tmp_val_str);/*注4*/

      memset(tmp_val_str,'\0',sizeof(tmp_val_str));
      memcpy(tmp_val_str,pICS_482102_N->LotNum5,sizeof(pICS_482102_N->LotNum5));
      setValueOfStr(recv_buff,"LotNum5",tmp_val_str);/*注5*/

      memset(tmp_val_str,'\0',sizeof(tmp_val_str));
      memcpy(tmp_val_str,pICS_482102_N->LotNum6,sizeof(pICS_482102_N->LotNum6));
      setValueOfStr(recv_buff,"LotNum6",tmp_val_str);/*复试注注*/

      memset(tmp_val_str,'\0',sizeof(tmp_val_str));
      memcpy(tmp_val_str,pICS_482102_N->TxnAmt,sizeof(pICS_482102_N->TxnAmt));
      setValueOfStr(recv_buff,"TxnAmt",tmp_val_str);/*金额       */ 

      memset(tmp_val_str,'\0',sizeof(tmp_val_str));
      memcpy(tmp_val_str,pICS_482102_N->MulTip,sizeof(pICS_482102_N->MulTip));
      setValueOfStr(recv_buff,"MulTip",tmp_val_str);/*倍数       */ 

      memset(tmp_val_str,'\0',sizeof(tmp_val_str));
      memcpy(tmp_val_str,pICS_482102_N->TrmCod,sizeof(pICS_482102_N->TrmCod));
  ...
  { /*失败*/
      /*获得错误返回包*/
      memcpy(pICS_482102_E,ics_recv_buff+sizeof(ICS_DEF_TOA),sizeof(ICS_DEF_482102_E_TC));
  
      /* 调用setValueOf函数填充 */
      /*格式:setValueOfStr(recv_buff,"display_zone",display_str);*/
      memset(tmp_val_str,'\0',sizeof(tmp_val_str));
      memcpy(tmp_val_str,pICS_482102_E->RspCod,sizeof(pICS_482102_E->RspCod));
      setValueOfStr(recv_buff,"RspCod",tmp_val_str);/*返回码       */

      memset(tmp_val_str,'\0',sizeof(tmp_val_str));
      memcpy(tmp_val_str,pICS_482102_E->RspMsg,sizeof(pICS_482102_E->RspMsg));
