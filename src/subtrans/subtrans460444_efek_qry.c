...
int ics_proc_460444_efek_qry(char *send_buff,char *recv_buff)
{
  /*说明：send_buff-上传报文；
          recv_buff-下传报文;
 ...
 */
  ...
  ICS_DEF_460444_I_EFEK_Qry  *pICS_REQUEST_I;
  ICS_DEF_460444_N_EFEK_Qry  *pICS_RESPONSE_N;
  ICS_DEF_DEFAULT_E  *pICS_RESPONSE_E;

  char      ics_send_buff[LEN_ICS_PROC_BUF];
  char      ics_recv_buff[LEN_ICS_PROC_BUF];
  char      ics_request_i_buff[21];
  char      ics_response_n_buff[650];
  ...
  pICS_REQUEST_I=(ICS_DEF_460444_I_EFEK_Qry *)ics_request_i_buff;
  pICS_RESPONSE_N=(ICS_DEF_460444_N_EFEK_Qry *)ics_response_n_buff;
...
flog( STEP_LEVEL,"--482101 接收[%s]-------------------------------",send_buff);
 ...
 /* 注意：填充数据最好按照结构定义先后顺序，以免出现数据覆盖问题 */
  ...
  len=sizeof(ICS_DEF_460444_I_EFEK_Qry);
  for(i=0;i<len;i++)
  {
  ...
  offset=offset+sizeof(ICS_DEF_460444_I_EFEK_Qry);
...
RETURN:
  ...
  if(memcmp(pICS_TOA->RspCod,"000000",sizeof(pICS_TOA->RspCod))==0)/*成功*/
  {
    ...
    memcpy(pICS_RESPONSE_N,ics_recv_buff+sizeof(ICS_DEF_TOA),sizeof(ICS_DEF_460444_N_EFEK_Qry));
