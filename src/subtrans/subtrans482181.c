...
int ics_proc_482181(char *send_buff,char *recv_buff)
{
  /*说明：send_buff-上传报文；
          recv_buff-下传报文;
 ...
 */
  ...
  ICS_DEF_482181_I	*pICS_482181_I;
  ICS_DEF_482181_N	*pICS_482181_N;
  ICS_DEF_482181_E	*pICS_482181_E;

  char			ics_send_buff[LEN_ICS_PROC_BUF];
  char			ics_recv_buff[LEN_ICS_PROC_BUF];
  char			ics_482181i_buff[38];
  char			ics_482181n_buff[118];
  char			ics_482181e_buff[75];
  ...
  /* STEP1-1:清理结构和变量 */
	...
	pICS_482181_I=(ICS_DEF_482181_I *)ics_482181i_buff;
	pICS_482181_N=(ICS_DEF_482181_N *)ics_482181n_buff;
	pICS_482181_E=(ICS_DEF_482181_E *)ics_482181e_buff;
	pICS_TIA=(ICS_DEF_TIA *)ics_tia_buff;
	pICS_TOA=(ICS_DEF_TOA *)ics_toa_buff;

	memset(ics_send_buff,'\0',sizeof(ics_send_buff));
	memset(ics_recv_buff,'\0',sizeof(ics_recv_buff));
	memset(ics_482181i_buff,'\0',sizeof(ics_482181i_buff));
	memset(ics_482181n_buff,'\0',sizeof(ics_482181n_buff));
	memset(ics_482181e_buff,'\0',sizeof(ics_482181e_buff));
...
flog( STEP_LEVEL,"--482181 接收[%s]-------------------------------",send_buff);
 ...
 /* 注意：填充数据最好按照结构定义先后顺序，以免出现数据覆盖问题 */
  ...
  strcpy(pICS_TIA->TTxnCd,"482181");
  strcpy(pICS_TIA->FeCod,"482181");
 ...
 /*将终端的交易渠道赋值进来*/
  ...
  len=sizeof(ICS_DEF_482181_I);
  for(i=0;i<len;i++)
  {
  	if(ics_482181i_buff[i]==0)
   		ics_482181i_buff[i]=' ';
  }
  memcpy(ics_send_buff+offset,ics_482181i_buff,len);
  offset=offset+sizeof(ICS_DEF_482181_I);
  ...
  /* 注意，<br>是页面显示的换行符号 */
		...
		memcpy(pICS_482181_N,ics_recv_buff+sizeof(ICS_DEF_TOA),sizeof(ICS_DEF_482181_N));
  	
  	memset(tmp_val_str2,0,sizeof(tmp_val_str2));
		memcpy(tmp_val_str2,pICS_482181_N->BalTyp,sizeof(pICS_482181_N->BalTyp));
		sprintf(tmp_val_str,"余额类型： %s  <br>",tmp_val_str2);
		strcat(display_str,tmp_val_str);
  	
  	memset(tmp_val_str2,0,sizeof(tmp_val_str2));
		memcpy(tmp_val_str2,pICS_482181_N->Balance,sizeof(pICS_482181_N->Balance));
  	...
  	memset(tmp_val_str2,0,sizeof(tmp_val_str2));
		memcpy(tmp_val_str2,pICS_482181_N->EffTime,8);
		sprintf(tmp_val_str,"余额有效期开始时间： %s <br>",tmp_val_str2);
		strcat(display_str,tmp_val_str);
		
  	memset(tmp_val_str2,0,sizeof(tmp_val_str2));
		memcpy(tmp_val_str2,pICS_482181_N->ExpireTime,8);
  	...
  	setValueOfStr(recv_buff,"display_zone",display_str);
		...
		memset(tmp_val_str,'\0',sizeof(tmp_val_str));
		memcpy(tmp_val_str,pICS_482181_N->BalTyp,sizeof(pICS_482181_N->BalTyp));
		setValueOfStr(recv_buff,"BalTyp",tmp_val_str);/*余额类型*/ 
  	
		memset(tmp_val_str,'\0',sizeof(tmp_val_str));
		memcpy(tmp_val_str,pICS_482181_N->Balance,sizeof(pICS_482181_N->Balance));
		setValueOfStr(recv_buff,"Balance",tmp_val_str);/*话费余额*/ 
  	
		memset(tmp_val_str,'\0',sizeof(tmp_val_str));
		memcpy(tmp_val_str,pICS_482181_N->EffTime,sizeof(pICS_482181_N->EffTime));
		setValueOfStr(recv_buff,"EffTime",tmp_val_str);/*余额有效期开始时间*/ 
		
		memset(tmp_val_str,'\0',sizeof(tmp_val_str));
		memcpy(tmp_val_str,pICS_482181_N->ExpireTime,sizeof(pICS_482181_N->ExpireTime));
		setValueOfStr(recv_buff,"ExpireTime",tmp_val_str);/*余额到期时间*/

  }
  else
  {	/*失败*/
		memcpy(pICS_482181_E,ics_recv_buff+sizeof(ICS_DEF_TOA),sizeof(ICS_DEF_482181_E));
  	...
  	memset(tmp_val_str2,0,sizeof(tmp_val_str2));
		memcpy(tmp_val_str2,pICS_482181_E->RspMsg,sizeof(pICS_482181_E->RspMsg));
...
flog( STEP_LEVEL,"**482181 返回[%s]******************************",recv_buff);
