 ...
 */
  ...
  typedef ICS_DEF_461502_I_PNTA ICS_DEF_REQUEST;
  typedef ICS_DEF_461502_N_PNTA ICS_DEF_RESPONSE_N;
 ...
 /* 注意：填充数据最好按照结构定义先后顺序，以免出现数据覆盖问题 */
  ...
  trim(tmpvalue);
  strcpy(pICS_REQUEST_I->AgtFlg,tmpvalue);
  
  memset(tmpvalue, 0, sizeof(tmpvalue));
  getValueOfStr(send_buff,"OActFg",tmpvalue);    /*2存折 4太平洋借记卡 付款账户性质*/
  trim(tmpvalue);
  strcpy(pICS_REQUEST_I->OActFg,tmpvalue);
  
  memset(tmpvalue, 0, sizeof(tmpvalue));
  getValueOfStr(send_buff,"AdnAmt",tmpvalue);    /*应收金额*/
  trim(tmpvalue);
  strcpy(pICS_REQUEST_I->AdnAmt,tmpvalue);
  
  memset(tmpvalue, 0, sizeof(tmpvalue));
  getValueOfStr(send_buff,"FinAccIn",tmpvalue);    /*银行入账帐号*/
  trim(tmpvalue);
  strcpy(pICS_REQUEST_I->FinAccIn,tmpvalue);
  
  memset(tmpvalue, 0, sizeof(tmpvalue));
  getValueOfStr(send_buff,"HoActNo",tmpvalue);   /*银行扣款账号*/
  trim(tmpvalue);
  strcpy(pICS_REQUEST_I->HoActNo,tmpvalue);
  
  memset(tmpvalue, 0, sizeof(tmpvalue));
  getValueOfStr(send_buff,"HoActNm",tmpvalue);   /*银行扣款户名*/
  trim(tmpvalue);
  strcpy(pICS_REQUEST_I->HoActNm,tmpvalue);
  ...
  trim(tmpvalue);
  strcpy(pICS_REQUEST_I->UdwDat,tmpvalue);
  
  memset(tmpvalue, 0, sizeof(tmpvalue));
  getValueOfStr(send_buff,"CclNo",tmpvalue);      /*销账号*/
  trim(tmpvalue);
  strcpy(pICS_REQUEST_I->CclNo,tmpvalue);
  
  memset(tmpvalue, 0, sizeof(tmpvalue));
  getValueOfStr(send_buff,"AdnKnd",tmpvalue);     /*通知书性质 1普通单 2汇总单 3交罚单 4批扣*/
  trim(tmpvalue);
