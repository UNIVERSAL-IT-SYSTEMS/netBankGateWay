...
int ics_proc_482118_unca(char *send_buff,char *recv_buff)
{
  /*说明：send_buff-上传报文；
          recv_buff-下传报文;
 ...
 */
  ...
  ICS_DEF_482118_I_MOB  *pICS_Request; /* ICS发送报文体 */
  ICS_DEF_482118_N_MOB  *pICS_Normal_Response; /* ICS发送报文头 */
  ...
  char      ics_i_buff[sizeof(ICS_DEF_482118_I_MOB)];
  char      ics_n_buff[sizeof(ICS_DEF_482118_N_MOB)];
  ...
  /* STEP1-1:清理结构和变量 */
  /* 将char数组映射成指针方便操作 */
  pICS_Request=(ICS_DEF_482118_I_MOB *) ics_i_buff;
  pICS_Normal_Response=(ICS_DEF_482118_N_MOB *)ics_n_buff;
 ...
 /* 注意：填充数据最好按照结构定义先后顺序，以免出现数据覆盖问题 */
  ...
  memset(tmpvalue, 0, sizeof(tmpvalue));
  getValueOfStr(send_buff,"CrdNo",tmpvalue);  /* 被充值号码 */
  trim(tmpvalue);
  strcpy(pICS_Request->CrdNo,tmpvalue);

  memset(tmpvalue, 0, sizeof(tmpvalue));
  getValueOfStr(send_buff,"TxnAmt",tmpvalue);  /* 被充值号码 */
  trim(tmpvalue);
  ...
  len=sizeof(ICS_DEF_482118_I_MOB);
  for(i=0;i<len;i++)
  {
    if(ics_i_buff[i]==0)
       ics_i_buff[i]=' ';
  ...
  offset=offset+sizeof(ICS_DEF_482118_I_MOB);
...
RETURN:
     ...
     ||memcmp(pICS_TOA->RspCod,"SC0000",sizeof(pICS_TOA->RspCod))==0)/*成功*/
  {
    ...
    memcpy(pICS_Normal_Response,ics_recv_buff+sizeof(ICS_DEF_TOA),sizeof(ICS_DEF_482118_N_MOB));
  ...
  flog( STEP_LEVEL,"**482118 返回[%s]******************************",recv_buff);
  return 0;
